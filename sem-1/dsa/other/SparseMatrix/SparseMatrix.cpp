...
const std::vector<int>& SparseMatrix::operator()(int i) const
{
    return m_mat[i];
}

std::vector<int>& SparseMatrix::operator()(int i)
{
    return m_mat[i];
}

const int& SparseMatrix::operator()(int i, int j) const
{
    return m_mat[i][j];
}
...
SparseMatrix SparseMatrix::create(std::vector<std::vector<int>> mat)
{
    ...
    for (std::size_t i{ 0 }; i < mat.size(); ++i)
        for (std::size_t j{ 0 }; j < mat[i].size(); ++j)
            if (mat[i][j])
                newSparse.insert({static_cast<int>(i), static_cast<int>(j), mat[i][j]});
